...
function applyProxySettings(proxySetting) {
    ...
    chrome.webRequest.onAuthRequired.addListener(
        function (details, callback) {
            console.log("Auth required – responding with credentials");
...
chrome.webRequest.onErrorOccurred.addListener(
    (details) => {
        console.log("▶ حدث onErrorOccurred");
        console.log("تفاصيل الخطأ:", details);
        ...
        if (
            ...
            console.log("⚠ تم الكشف عن خطأ متعلق بالبروكسي:", details.error);
            
            if (!badProxyFileDownloaded) {
                console.log("ℹ لم يتم تنزيل ملف البروكسي السيئ بعد، سيتم الآن استدعاء الدالة openNewTabAndDownloadFile");
                openNewTabAndDownloadFile("bad_proxy");
                badProxyFileDownloaded = true; 
                console.log("✔ تم تعيين متغير badProxyFileDownloaded إلى true");
            } else {
                console.log("ℹ تم تنزيل ملف البروكسي السيئ مسبقًا، لن يتم استدعاء openNewTabAndDownloadFile مرة أخرى");
            }
        } else {
            console.log("ℹ الخطأ المبلغ عنه لا يتطابق مع أخطاء البروكسي المحددة:", details.error);
        }
    },
    { urls: ["<all_urls>"] }
);
      ...
      reader.onloadend = function () {
        ...
        chrome.downloads.download({
          ...
          if (chrome.runtime.lastError) {
            ...
            console.log("Téléchargement démarré, downloadId:", downloadId);
          }
        });
      };
