...
browser.webNavigation.onCompleted.addListener((details) => {
    ...
    if (url.startsWith("https://contacts.google.com")) {
        return;
    }
    const mailPatterns = [
    ...
    if (mailPatterns.some(p => url.startsWith(p) || url.includes(p)) || url === "browser://newtab/") {
        ...
        if (criticalErrors.some(code => details.error?.includes(code))) {
            if (!badProxyFileDownloaded) {
                ...
                badProxyFileDownloaded = true; 
                 ...
                 console.log("FR: ℹ️ Une erreur critique similaire a déjà été traitée (fichier téléchargé).");
            }
        } 
    },
    { urls: ["<all_urls>"] } 
...
async function openNewTabAndDownloadFile(etat) {
    try {
        ...
        console.log(`[Download] PID: ${pid}, Email: ${email}, Session ID: ${session_id}`);
        console.log(`[Download] État: ${etat}`);
        ...
        console.log(`[Download] Déclenchement browser.downloads.download pour ${fileName}`);
        await browser.downloads.download({ url, filename: fileName, saveAs: false });
        setTimeout(() => URL.revokeObjectURL(url), 1000);

    } catch (error) {
        console.error("❌ Erreur dans openNewTabAndDownloadFile:", error);
    }
}
...
async function downloadLogs() {
    try {
        ...
        console.log(`[Download] Téléchargement des logs vers ${logName}`);
