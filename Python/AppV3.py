    ...
    def run(self):
        ...
        while logs_running: 
            if self.logs:
                ...
                print("😴 No logs available. Sleeping for 5 second...")
                time.sleep(5)  
...
def checkVersion():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # script_dir =  E:\Lastversion\Programme-main\Python\AppV2.pyc 
            # client_version_path_Python =  E:\Lastversion\Programme-main\Python\version.txt 
            # client_version_path_Python = os.path.join(script_dir, '..','..',"Programme-main", "Python","version.txt")
            # # client_version_path_Extention =  E:\Lastversion\Programme-main\tools\version.txt 
            # client_version_path_Extention = os.path.join(script_dir, "Programme-main", "tools","version.txt")
            # # client_version_path_Extention =  E:\Lastversion\Programme-main\interface\version.txt 
...
def start_extraction(window, data_list, entered_number):
    ...
    extraction_thread.finished.connect(lambda: read_result_and_update_list(window))
...
class ExtractionThread(QThread):
    ...
    def run(self):
        ...
        while remaining_emails or process_pids:  
            if self.stop_flag:  
                log_message("Processing stopped by the user.") 
                break
            ...
            if len(process_pids) < self.entered_number and remaining_emails:
                ...
                log_message(f"Traitement de l'email : {email_value}") 
                try:
                    ...
                    if len(logs_subdirs) > 4:
                        ...
                        for dir_to_delete in to_delete:
                            try:
                                ...
                                log_message(f"Erreur lors de la suppression de {dir_to_delete} : {e}")
...
class MainWindow(QMainWindow):
    def __init__(self, json_data):
        ...
        # Appliquer un style professionnel aux scrollAreas
...
def main():
    ...
    #  mon besoin une solition de mon probleme d abord fais une analyse de mon code apres donner  solition ne arabe detaille p
