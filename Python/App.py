...
def encrypt_date(date_str: str, fernet: Fernet) -> str:
    return fernet.encrypt(date_str.encode()).decode("utf-8")




def decrypt_date(encrypted_str: str, fernet: Fernet) -> str:
    return fernet.decrypt(encrypted_str.encode()).decode("utf-8")





key =  "zBb1hDM4nU8NQCHjKd8Vkfz86S4qTaXmFh5n0sguDCg="
fernet = Fernet(key)
...
class MainWindow(QMainWindow):
    ...
    def on_submit_button_clicked(self, window):
        ...
        if os.path.exists(session_path):
            try:
                ...
                if not encrypted:
                    print("[SESSION WARNING] Le fichier session.txt est vide.")
                else:
                    decrypted = decrypt_date(encrypted, fernet)

                    # Extraction du username et de la date, comme dans main()
                    if "::" in decrypted:
                        ...
                        try:
                            ...
                            print(f"[SESSION DATE ERROR] Format de date invalide: {e}")
                    else:
                        print("[SESSION ERROR] Format invalide dans session.txt")
            except Exception as e:
                print(f"[SESSION ERROR] {e}")
        else:
            print("[SESSION WARNING] Le fichier session.txt n'existe pas.")

        if not session_valid:
            print("[SESSION] Session invalide : ouverture de LoginWindow...")

            # 🔁 Ouvrir LoginWindow (en le stockant pour éviter qu’il soit détruit)
            self.login_window = LoginWindow()
            self.login_window.setFixedSize(1700, 1005)

            self.login_window.show()

            # ❌ Fermer la fenêtre actuelle
            print("[SESSION] Fermeture de MainWindow...")
            self.close()

            # 🧹 Vider le fichier session
            try:
                with open(session_path, "w") as f:
                    f.write("")
                print("[SESSION] session.txt vidé.")
            except Exception as e:
                print(f"[SESSION CLEAN ERROR] {e}")
        ...
        for i in range(self.scenario_layout.count()):
            ...
            if widget:
                if i != self.scenario_layout.count() - 1:
                    widget.setStyleSheet("background-color: #ffffff; border: 1px solid #669bbc; border-radius: 8px;")
    ...
    def update_scenario(self, template_name, state):
        ...
        if template_frame:
            ...
            new_template.setMaximumWidth(720)  # ← Ajout ici (ajuste selon ton besoin)
...
class LoginWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Gmail Process Automation")
        ...
        if os.path.exists(session_path):
            try:
                ...
                if "::" in decrypted:
                    ...
                    if now - last_session < timedelta(days=2):
                        return os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "interface.ui"))
    ...
    def initialize_login_ui(self):
        ...
        self.login_input = self.findChild(QTextEdit, "loginInput")
        self.password_input = self.findChild(QTextEdit, "passwordInput")
    ...
    def handle_login(self):
        """
        Gère le processus de connexion avec une meilleure gestion des permissions booléennes
        """
        username = self.login_input.toPlainText().strip() if self.login_input else ""
        password = self.password_input.toPlainText().strip() if self.password_input else ""
        ...
        login_url = "http://localhost/auth-api/login.php"
        auth_check_url = "http://localhost/auth-api/check-permission.php"
        ...
        try:
            ...
            data = response.json()
            print(f"[DEBUG] Réponse login: {data}")

            if response.status_code != 200:
                self.erreur_label.setText(f"Erreur serveur (HTTP {response.status_code})")
                self.erreur_label.show()
                return

            if "success" not in data:
                self.erreur_label.setText("Échec d'authentification : Réponse serveur invalide")
                self.erreur_label.show()
                return

            # 2. Vérification des permissions (adaptée pour gérer booléen/string)
            permission_response = requests.post(auth_check_url, json={"username": username})
            permission_data = permission_response.json()
            print(f"[DEBUG] Réponse permissions: {permission_data}")

            # Nouvelle vérification qui accepte True/1/"1"
            has_permission = False
            if 'permission' in permission_data:
                perm_value = permission_data['permission']
            ...
            if not has_permission:
                self.erreur_label.setText("Accès refusé : Privilèges insuffisants")
                self.erreur_label.show()
                return

            # 3. Suite du processus si autorisé
            self.erreur_label.hide()
            
            # Sauvegarde session
            session_data = f"{username}::{datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}"
            encrypted = encrypt_date(session_data, fernet)
            
            os.makedirs(APPDATA_DIR, exist_ok=True)
            with open(os.path.join(APPDATA_DIR, "session.txt"), "w") as f:
                f.write(encrypted)
            ...
            self.main_window.setFixedSize(1700, 1005)
            self.main_window.setWindowTitle("Gestion Automatisée Gmail")
            self.main_window.stopButton.clicked.connect(lambda: stop_all_processes(self.main_window))
            self.main_window.show()
            self.close()

        except requests.exceptions.RequestException as e:
            self.erreur_label.setText(f"Erreur réseau : {str(e)}")
            self.erreur_label.show()
        except Exception as e:
            self.erreur_label.setText(f"Erreur inattendue : {str(e)}")
            self.erreur_label.show()
...
def main():
    ...
    session_path = os.path.join(APPDATA_DIR, "session.txt")
    session_valid = False
    username = None  # Pour stocker le nom d'utilisateur extrait
    ...
    if os.path.exists(session_path):
        try:
            ...
            if not encrypted:
                print("[SESSION] Fichier de session vide")
            else:
                decrypted = decrypt_date(encrypted, fernet)
                ...
                if "::" in decrypted:
                    ...
                    try:
                        ...
                        # Vérification de la validité temporelle (48h max)
                        if (now - last_session) < timedelta(days=2):
                            # Vérification des permissions (nouvelle partie)
                            auth_check_url = "http://localhost/auth-api/check-permission.php"
                            try:
                                ...
                                # Vérification flexible des permissions
                                if 'permission' in permission_data:
                                    perm = permission_data['permission']
                                    has_permission = perm is True or str(perm) == "1"
                                    
                                    if has_permission:
                                        session_valid = True
                                        print(f"[SESSION] Session valide pour {username}")
                                    else:
                                        print(f"[PERMISSION] Accès refusé pour {username}")
                                else:
                                    print("[PERMISSION] Réponse de permission invalide")
                            except requests.RequestException as e:
                                print(f"[API ERROR] Erreur de vérification: {e}")
                        else:
                            print(f"[SESSION] Session expirée (date: {date_str})")
                    except ValueError as e:
                        print(f"[SESSION] Format de date invalide: {e}")
                else:
                    print("[SESSION] Format de session invalide (username::date manquant)")
        except Exception as e:
            print(f"[SESSION ERROR] {e}")
    else:
        print("[SESSION] Aucun fichier de session trouvé")
    ...
    # Chargement de l'interface appropriée
    if session_valid:
        try:
            json_path = os.path.join(script_dir, '..', "Tools", "action.json")
            with open(json_path, "r", encoding='utf-8') as file:
            ...
            if json_data:
                ...
                window.setFixedSize(1700, 1005)
                window.stopButton.clicked.connect(lambda: stop_all_processes(window))
                window.setWindowTitle("Gestion Automatisée Gmail")
                window.show()
            else:
                raise ValueError("Fichier de configuration vide")
        except Exception as e:
            print(f"[CONFIG ERROR] {e}")
            # Fallback à la fenêtre de login si problème de configuration
            window = LoginWindow()
            window.setFixedSize(1700, 1005)
            window.show()
    else:
