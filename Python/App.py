...
base_directory = os.path.join(script_dir, 'Tools-main', 'ExtensionEmail')
template_directory = os.path.join(script_dir, 'Tools-main', 'ExtensionTemplate')
...
def DownloadFile(type_version):
    local_filename = os.path.join(script_dir, "Tools.zip")
    ...
    url = "https://github.com/Azedize/Tools/archive/refs/heads/main.zip"
...
def checkVersion():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            server_version = data.get("version_python")
            if not server_version:
                print("❌ [ERROR] 'version_python' key is missing in server response.")
                os.system("pause")
                exit()
            
            client_version_path = os.path.join(script_dir, "Tools-main", "version.txt")
            client_version = ""
            if os.path.exists(client_version_path):
                with open(client_version_path, 'r') as file:
                    client_version = file.read().strip()

            print(f"🌐 [INFO] Server Global Version: {server_version}")
            print(f"💻 [INFO] Local Client Version: {client_version}")

            if server_version != client_version:
                version_updates["version_python"] = server_version

            # Vérification de la version de l'interface
            server_interface_version = data.get("version_Extention")
            if not server_interface_version:
                print("❌ [ERROR] 'version_Extention' key is missing in server response.")
                os.system("pause")
                exit()

            client_version_Extention_path = os.path.join(script_dir,"Tools-main", "interface", "version.txt")
            client_interface_version = ""
            if os.path.exists(client_version_Extention_path):
                with open(client_version_Extention_path, 'r') as file:
                    client_interface_version = file.read().strip()

            print(f"🌐 [INFO] Server Interface Version: {server_interface_version}")
            print(f"💻 [INFO] Local Interface Version: {client_interface_version}")

            if server_interface_version != client_interface_version:
                version_updates["version_Extention"] = server_interface_version
...
def add_pid_to_text_file(pid, email):
    text_file_path = os.path.join(base_directory, email, "Tools-main/data.txt")
        ...
        while remaining_emails or process_pids:  
            ...
            if len(process_pids) < self.entered_number and remaining_emails:
                ...
                try:
                    ...
                    command = [
                        self.chrome_path,
                        f"--user-data-dir={os.path.join(script_dir, 'Tools-main', 'Profiles', profile_email)}",
...
class MainWindow(QMainWindow):
    def __init__(self, json_data):
        ...
        ui_path = os.path.join(script_dir,"Tools-main",  "interface"  , "interface.ui")
        ...
        default_icon_path = os.path.join(script_dir, "Tools-main", "icons")
    ...
    def create_option_button(self, state):
        ...
        default_icon_path = os.path.join(script_dir, "Tools-main", "icons", "icon.png")
